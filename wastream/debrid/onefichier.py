...
class OneFichierService(BaseDebridService):
    ...
    async def check_cache_and_enrich(self, results: List[Dict], api_key: str, config: Dict, timeout_remaining: float, user_season: Optional[str] = None, user_episode: Optional[str] = None, user_hosts: Optional[List[str]] = None) -> List[Dict]:
        ...
        if not filtered_results:
            debrid_logger.debug("[1Fichier] No supported hosts")
            return []
        ...
        cache_logger.debug(f"[1Fichier] Done in {elapsed:.1f}s: {len(cached_results)} results (all marked cached)")
        ...
        for attempt in range(settings.DEBRID_MAX_RETRIES):
            try:
                ...
                if should_retry:
                    ...
                    debrid_logger.error(f"[1Fichier] Max HTTP retries ({settings.DEBRID_HTTP_ERROR_MAX_RETRIES})")
                    return "RETRY_ERROR"

                http_error_count = 0

                if response.status_code == 401:
                    debrid_logger.error("[1Fichier] Invalid API key (401)")
                    return "FATAL_ERROR"

                if response.status_code == 403:
                    debrid_logger.error("[1Fichier] Premium account required (403)")
                    return "FATAL_ERROR"

                if response.status_code == 404:
                    debrid_logger.error("[1Fichier] Link not found (404)")
                    return "LINK_DOWN"

                if response.status_code == 410:
                    debrid_logger.error("[1Fichier] Link removed (410)")
                    return "LINK_DOWN"
                ...
                if direct_link:
                    debrid_logger.debug("[1Fichier] Converted")
                    return direct_link
                else:
