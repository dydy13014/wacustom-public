...
class BaseDebridService(ABC):
    ...
    def group_identical_links(self, results: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for result in results:
            group_key = (
                ...
                result.get("year", ""),
                # NOTE: source intentionally excluded so the same file
                # from different private trackers is treated as one group
            )
    ...
    def deduplicate_by_exact_link(self, results: List[Dict]) -> List[Dict]:
        ...
        for result in results:
            ...
            # For torrents, deduplicate by infohash first (same torrent on multiple trackers)
            if infohash:
        ...
        if len(results) != len(deduplicated):
            cache_logger.debug(f"Deduplicated {len(results)} → {len(deduplicated)} (exact links + infohash)")
