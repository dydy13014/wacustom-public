...
async def _check_single_source(name: str, url: Optional[str]) -> SourceStatus:
    ...
    try:
        if name == "Darki-API":
            ...
            # Instance publique : l'hebergeur n'a PAS de cle pour ces trackers
            # (chaque utilisateur apporte la sienne). On sonde donc sans cle et
            # on considere le tracker en ligne des qu'il repond quelque chose
            # d'intelligible — un 401/403 signifie « service joignable, cle
            # absente ou invalide », ce qui est exactement l'etat attendu ici
            # et reste une information de disponibilite valable.
            tracker_url = normalize_tracker_url(name, url) + "?t=caps"
            headers = {
